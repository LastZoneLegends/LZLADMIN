...
export default function Tournaments() {
  ...
  const getPrizePerMember = (totalPrize, slotNumber) => {
    ...
    return teamMembers.length > 0 ? totalPrize / teamMembers.length : 0;
  };
  ...
  const getTotalEarnings = (participant) => {
    ...
    if (isTeamMode()) {
      // Team mode: prize split among team members
      if (isInWinningSlot(participant, 'first')) {
        total += getPrizePerMember(selectedTournament?.prize1 || 0, participant.slotNumber);
      }
      if (isInWinningSlot(participant, 'second')) {
        total += getPrizePerMember(selectedTournament?.prize2 || 0, participant.slotNumber);
      }
      if (isInWinningSlot(participant, 'third')) {
        total += getPrizePerMember(selectedTournament?.prize3 || 0, participant.slotNumber);
      }
    } else {
      // Solo mode: full prize to winner
      if (winners.first === participant.odeuId) total += selectedTournament?.prize1 || 0;
      if (winners.second === participant.odeuId) total += selectedTournament?.prize2 || 0;
      if (winners.third === participant.odeuId) total += selectedTournament?.prize3 || 0;
    }
  ...
  const handleAnnounceResult = async () => {
    ...
    try {
      ...
      for (const participant of participantResults) {
        const newEarnings = getTotalEarnings(participant);
        const previousEarnings = participant.previousEarnings || 0;
        const difference = newEarnings - previousEarnings;

        if (difference !== 0 && participant.odeuUserId) {
          // Update user's wallet (positive = credit, negative = debit)
          await updateDoc(doc(db, 'users', participant.odeuUserId), {
            walletBalance: increment(difference),
            winningBalance: increment(difference),
            totalWinnings: increment(difference > 0 ? difference : 0) // Only add positive to total
          });
